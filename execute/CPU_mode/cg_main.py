from functions.data_preparation import get_data_channel
...
if __name__ == '__main__':
    ...
    # ------------------------------------- Establish Network ----------------------------------------------
    # PATH = '../../SAD_pretrained/CG/ber_model/tx%i/rx%i/rate%i/iterations%i/project_times%i/batch_size%i/rnn_hidden_size%i/sparse%.3f' % (TX, RX, RATE,
    #                                                                                                         ITERATIONS,
    #                                                                                                         PROJECT_TIMES,
    #                                                                                                         BATCH_SIZE,
    #                                                                                                         RNN_HIDDEN_SIZE,
    #                                                                                                         SPARSITY)
    model = my_models.CGDetectorModified(TX, RNN_HIDDEN_SIZE, PROJECT_TIMES, SPARSITY)
    ...
    for epoch in range(EPOCHS):
        ...
        for i, data in enumerate(train_loader, 0):
            ...
            x, h, outputs, _ = model(A, b, ITERATIONS)
    ...
    # --------------------------------------------------- Test ---------------------------------------------------------
    # sd = ProjectionXI.CGDetector(TX)
    sd = my_models.CGMethod(TX)
    with torch.no_grad():
        model.eval()
        ...
        for i, data in enumerate(test_loader, 0):
            ...
            x, h, outputs, pre = model(A, b, ITERATIONS)
            loss = loss_cal.weighted_mse(outputs, label, RATE)

            results = pre[3]
            predict_x, _, _ = results

            loss_mse_p = loss_cal.common_loss(predict_x, label, RATE)
            loss_ml_p = loss_cal.ml_loss_single(predict_x, y, h_com)
            x_sd = sd(A, b, results, 5)
        ...
        print('CG mse loss: %.5f' % (sd_loss / test_steps))
        print('projection ML loss: %.5f' % (projection_ml_loss / test_steps))
        print('CG ML loss: %.5f' % (sd_ml_loss / test_steps))
    ...
    with torch.no_grad():
        ...
        for ebn0 in TEST_EBN0:
            ...
            with torch.no_grad():
                ...
                prediction_cg = []
                for i, data in enumerate(test_loader, 0):
                    A, b, label, data_real, data_imag, y, h_com = data['A'], data['b'], data['label'], data[
                        'data_real'], data['data_imag'], data['y'], data['h']
                    inputs = (y, h_com)
                    x, h, outputs, pre = model(A, b, ITERATIONS)
                    loss = loss_cal.weighted_mse(outputs, label, RATE)
                    ...
                    x_cg = sd(A, b, results, ITERATIONS)
                ...
                prediction_cg = torch.cat(prediction_cg).cpu().numpy()
                prediction_cg = ((prediction_cg + LENGTH - 1) / 2).round()
                ber_cg = gray_ber(prediction_cg, test_Data_real, test_Data_imag, rate=RATE)
                BER_CG += [ber_cg]
    ...
    scio.savemat(PATH+str('/BER_CG2.mat'), {'ber_cg': BER_CG})
